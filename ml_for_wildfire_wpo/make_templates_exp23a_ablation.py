"""Makes templates for Experiment 23a (first step of ablation)."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment23a_ablation/templates'
)
...
PREDICTOR_TYPE_STRINGS = [
    'gfs_2d', 'gfs_3d', 'lagged_targets', 'gfs_forecast_targets'
]
...
def _run():
    ...
    for i in range(len(PREDICTOR_TYPE_STRINGS)):
        ...
        if PREDICTOR_TYPE_STRINGS[i] in ['gfs_2d', 'gfs_3d']:
        ...
        if PREDICTOR_TYPE_STRINGS[i] == 'gfs_3d':
            ...
            option_dict[chiu_net_pp_arch.LAGTGT_DIMENSIONS_KEY] = numpy.array(
                [160, 160, 1, 7], dtype=int
            )

        model_object = chiu_net_pp_arch.create_model(option_dict)
        output_file_name = '{0:s}/predictor_type={1:s}/model.keras'.format(
            OUTPUT_DIR_NAME, PREDICTOR_TYPE_STRINGS[i]
        )
