...
def _add_bui_and_fwi_to_tensors(
        prediction_tensor, target_tensor_no_mask,
        dmc_index, dc_index, isi_index, expect_ensemble):
    """Adds build-up index (BUI) and fire-weather index (FWI) to each tensor.

    E = number of examples (time steps)
    M = number of rows in grid
    N = number of columns in grid
    T = number of target fields, not including BUI or FWI
    S = ensemble size
    ...
    :param expect_ensemble: Boolean flag, indicating whether to expect
        ensemble or deterministic predictions.
    :return: prediction_tensor: Same as input but with extra BUI and FWI
        channels at the end.  This tensor has dimensions E x M x N x (T + 2) x S
        or E x M x N x (T + 2).
    :return: target_tensor_no_mask: Same as input but with extra BUI and FWI
        channels at the end.  This tensor has dimensions E x M x N x (T + 2).
    """
...
def dual_weighted_crpss_constrained_bui_fwi_together(
        channel_weights, dmc_index, dc_index, isi_index, function_name,
        max_dual_weight_by_channel=None, test_mode=False):
    """Creates DWCRPSS loss function with constrained BUI and FWI only.
    ...
    def loss(target_tensor, prediction_tensor):
        ...
        prediction_tensor, target_tensor_no_mask = _add_bui_and_fwi_to_tensors(
            prediction_tensor=prediction_tensor,
            target_tensor_no_mask=target_tensor_no_mask,
            dmc_index=dmc_index,
            dc_index=dc_index,
