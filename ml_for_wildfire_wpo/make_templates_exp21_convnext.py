"""Makes templates for Experiment 21 with ConvNext.

Same as Experiment 20, but I throw out 75% of patches in the generator.
"""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment21_convnext/templates'
)
...
OPTIMIZER_FUNCTION = keras.optimizers.AdamW(gradient_accumulation_steps=int(25 * 44 * 0.1))
OPTIMIZER_FUNCTION_STRING = (
    'keras.optimizers.AdamW(gradient_accumulation_steps=int(25 * 44 * 0.1))'
)
        ...
        for j in range(len(SPECTRAL_COMPLEXITIES_AXIS2)):
            for k in range(len(USE_LEAD_TIME_AS_PRED_FLAGS_AXIS3)):
                ...
                output_file_name = (
                    ...
                    'spectral-complexity={2:03d}_'
                    'use-lead-time-as-predictor={3:d}/'
                    'model.keras'
                ).format(
                    OUTPUT_DIR_NAME,
                    PREDICTOR_TIME_STRATEGIES_AXIS1[i],
