...
def _check_index_args(dmc_index, dc_index, isi_index):
    ...
    :param dmc_index: See documentation for `dual_weighted_mse`.
    :param dc_index: Same.
...
def _add_bui_to_tensors(prediction_tensor, target_tensor_no_mask,
    ...
    tgt_dmc = K.maximum(target_dmc_tensor, K.epsilon())
    tgt_dc = K.maximum(target_dc_tensor, K.epsilon())
    ...
    pred_dmc = K.maximum(predicted_dmc_tensor, K.epsilon())
    pred_dc = K.maximum(predicted_dc_tensor, K.epsilon())
...
def dual_weighted_mse(
        channel_weights, dmc_dc_isi_indices_for_constraints, function_name,
        max_dual_weight_by_channel=None, expect_ensemble=True, test_mode=False):
    """Creates DWMSE loss function.

    K = number of output channels (target variables)

    K does not include constrained target variables (BUI, FWI, and DSR), which
    might be computed on the fly (if `dmc_dc_isi_indices_for_constraints is not
    None`).

    :param channel_weights: numpy array of channel weights.  If
        `dmc_dc_isi_indices_for_constraints is None`, this array have length K.
        Otherwise, this array must have length K + 3, where the last three
        weights correspond to BUI, FWI, and DSR -- in that order.
    :param dmc_dc_isi_indices_for_constraints: length-3 numpy array of indices
        for constrained target variables.  This tells the function that
        DMC predictions and targets can be found at
        target_tensor[:, :, :, dmc_dc_isi_indices_for_constraints[0], ...] and
        prediction_tensor[:, :, :, dmc_dc_isi_indices_for_constraints[0], ...];
        DC predictions and targets can be found at
        target_tensor[:, :, :, dmc_dc_isi_indices_for_constraints[1], ...] and
        prediction_tensor[:, :, :, dmc_dc_isi_indices_for_constraints[1], ...];
        while ISI predictions and targets can be found at
        target_tensor[:, :, :, dmc_dc_isi_indices_for_constraints[2], ...] and
        prediction_tensor[:, :, :, dmc_dc_isi_indices_for_constraints[2], ...].
    :param function_name: Function name (string).
    :param max_dual_weight_by_channel: numpy array of maximum dual weights.  If
        `dmc_dc_isi_indices_for_constraints is None`, this array have length K.
        Otherwise, this array must have length K + 3, where the last three
        weights correspond to BUI, FWI, and DSR -- in that order.
    :param expect_ensemble: Boolean flag.  If True, will expect
    ...
    if dmc_dc_isi_indices_for_constraints is None:
        dmc_index = None
        dc_index = None
        isi_index = None
    else:
        error_checking.assert_is_numpy_array(
            dmc_dc_isi_indices_for_constraints,
...
def dual_weighted_crps(
    ...
    """Creates DWCRPS loss function.

    :param channel_weights: See documentation for `dual_weighted_mse`.
    :param dmc_dc_isi_indices_for_constraints: Same.
...
def dual_weighted_msess_all_constraints(
        channel_weights, dmc_dc_isi_indices_for_constraints, function_name,
        max_dual_weight_by_channel=None, test_mode=False):
    """Creates DWMSESS loss function with constrained BUI, FWI, and DSR.
    ...
    def loss(target_tensor, prediction_tensor):
        ...
        relevant_prediction_tensor = K.expand_dims(prediction_tensor, axis=-1)
        mask_weight_tensor = K.expand_dims(mask_weight_tensor, axis=-1)
        ...
        return (actual_dwmse - gfs_dwmse) / gfs_dwmse
