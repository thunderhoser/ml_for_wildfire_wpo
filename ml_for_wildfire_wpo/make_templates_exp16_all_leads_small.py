"""Makes templates for Experiment 16-small (all lead times up to 10 days)."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment16_all_leads_small/templates'
)
...
DEFAULT_OPTION_DICT = {
    ...
    chiu_net_pp_arch.NUM_LEVELS_KEY: 4,
    ...
    chiu_net_pp_arch.GFS_ENCODER_DROPOUT_RATES_KEY: numpy.full(5, 0.),
    ...
    chiu_net_pp_arch.LAGTGT_ENCODER_DROPOUT_RATES_KEY: numpy.full(5, 0.),
    chiu_net_pp_arch.DECODER_NUM_CONV_LAYERS_KEY: numpy.full(4, 2, dtype=int),
    chiu_net_pp_arch.DECODER_NUM_CHANNELS_KEY: numpy.array(
        [40, 58, 75, 93], dtype=int
    ),
    chiu_net_pp_arch.UPSAMPLING_DROPOUT_RATES_KEY: numpy.full(4, 0.),
    chiu_net_pp_arch.SKIP_DROPOUT_RATES_KEY: numpy.full(4, 0.),
...
def _run():
    ...
    for i in range(len(PREDICTOR_TIME_STRATEGIES)):
        ...
        option_dict.update({
            ...
            chiu_net_pp_arch.GFS_3D_DIMENSIONS_KEY: numpy.array(
                [81, 81, 2, num_gfs_hours, 5], dtype=int
            ),
            chiu_net_pp_arch.GFS_2D_DIMENSIONS_KEY: numpy.array(
                [81, 81, num_gfs_hours, 5], dtype=int
            ),
            chiu_net_pp_arch.LAGTGT_DIMENSIONS_KEY: numpy.array(
                [81, 81, num_target_days, 6], dtype=int
            ),
        })
