"""Makes templates for Experiment 8 (ablation)."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment08_ablation/templates'
)

# TODO(thunderhoser): I still need to do the DSR fuckery in the loss function!
...
DEFAULT_OPTION_DICT = {
    ...
    chiu_net_pp_arch.GFS_ENCODER_NUM_CHANNELS_KEY: numpy.array(
        [20, 30, 40, 50, 60, 70, 80], dtype=int
    ),
...
ERA5_VARIABLE_COUNTS_AXIS3 = numpy.array([0, 1, 7], dtype=int)
GFS_TARGET_LEAD_TIME_COUNTS_AXIS4 = numpy.array([0, 2], dtype=int)
...
def _run():
    ...
    for i in range(GFS_PRESSURE_LEVEL_COUNTS_AXIS1):
        for j in range(GFS_2D_VARIABLE_COUNTS_AXIS2):
            for k in range(ERA5_VARIABLE_COUNTS_AXIS3):
                for l in range(GFS_TARGET_LEAD_TIME_COUNTS_AXIS4):
                    option_dict = copy.deepcopy(DEFAULT_OPTION_DICT)

                    these_dim_3d = numpy.array([
                        265, 537,
                        GFS_PRESSURE_LEVEL_COUNTS_AXIS1[i], NUM_GFS_LEAD_TIMES,
                        5
                    ], dtype=int)

                    these_dim_2d = numpy.array([
                        265, 537, NUM_GFS_LEAD_TIMES,
                        GFS_2D_VARIABLE_COUNTS_AXIS2[j]
                    ], dtype=int)

                    these_dim_era5 = numpy.array([
                        265, 537, ERA5_VARIABLE_COUNTS_AXIS3[k]
                    ], dtype=int)
                    ...
                    output_file_name = (
                        '{0:s}/num-gfs-pressure-levels={1:d}_'
                        'num-gfs-2d-vars={2:d}_'
                        'num-era5-constants={3:d}_'
                        'num-gfs-target-leads={4:d}/'
                        'model.keras'
                    ).format(
                        OUTPUT_DIR_NAME,
                        GFS_PRESSURE_LEVEL_COUNTS_AXIS1[i],
                        GFS_2D_VARIABLE_COUNTS_AXIS2[j],
                        ERA5_VARIABLE_COUNTS_AXIS3[k],
                        GFS_TARGET_LEAD_TIME_COUNTS_AXIS4[l]
                    )
