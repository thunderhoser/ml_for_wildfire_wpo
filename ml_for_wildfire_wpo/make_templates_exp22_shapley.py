"""Makes templates for Experiment 22 (simple NNs for Shapley analysis)."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment22_shapley/templates'
)
...
NUM_MODEL_LEADS_AXIS1 = numpy.array([1, 14], dtype=int)
NUM_MODEL_LEADS_TO_NUM_GFS_HOURS = {
    1: 6,
    14: 14
}
NUM_MODEL_LEADS_TO_NUM_TARGET_DAYS = {
    1: 5,
    14: 7
}

USE_LAGGED_TARGET_FLAGS_AXIS2 = numpy.array([False, True], dtype=bool)
RESID_PREDICTION_FLAGS_AXIS3 = numpy.array([False, True], dtype=bool)
...
def _run():
    ...
    for i in range(len(NUM_MODEL_LEADS_AXIS1)):
        for j in range(len(USE_LAGGED_TARGET_FLAGS_AXIS2)):
            for k in range(len(RESID_PREDICTION_FLAGS_AXIS3)):
                num_gfs_hours = (
                    NUM_MODEL_LEADS_TO_NUM_GFS_HOURS[NUM_MODEL_LEADS_AXIS1[i]]
                )
                ...
                option_dict.update({
                    ...
                    chiu_net_pp_arch.GFS_3D_DIMENSIONS_KEY: numpy.array(
                        [160, 160, 1, num_gfs_hours, 5], dtype=int
                    ),
                    chiu_net_pp_arch.GFS_2D_DIMENSIONS_KEY: numpy.array(
                    ...
                    chiu_net_pp_arch.USE_LEAD_TIME_AS_PRED_KEY:
                        NUM_MODEL_LEADS_AXIS1[i] > 1,
                    chiu_net_pp_arch.PREDN_BASELINE_DIMENSIONS_KEY:
                ...
                output_file_name = (
                    ...
                    OUTPUT_DIR_NAME,
                    NUM_MODEL_LEADS_AXIS1[i],
                    int(USE_LAGGED_TARGET_FLAGS_AXIS2[j]),
                    int(RESID_PREDICTION_FLAGS_AXIS3[k])
                )
